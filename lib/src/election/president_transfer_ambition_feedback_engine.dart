...
import 'president_financial_discipline_feedback_engine.dart';
...
class PresidentTransferAmbitionFeedbackEngine {
  const PresidentTransferAmbitionFeedbackEngine({
    this.m20Engine = const PresidentFinancialDisciplineFeedbackEngine(),
  ...
  final PresidentFinancialDisciplineFeedbackEngine m20Engine;
  ...
  PresidentTransferAmbitionFeedbackReport simulate({
    ...
    final m20Baseline = m20Engine.simulate(
      clubs: clubs,
      leagues: leagues,
      config: config,
    ...
    if (!m20Baseline.converged || m20Baseline.cycleDetected) {
      throw StateError('M21 requires a converged M20 baseline.');
    }

    var inputReport = m20Baseline.finalReport;
    var finalReport = inputReport;
    var converged = false;
    var cycleDetected = false;
    final iterations = <PresidentTransferAmbitionFeedbackIteration>[];
    ...
    for (var iteration = 1; iteration <= maxIterations; iteration++) {
      ...
      iterations.add(
        PresidentTransferAmbitionFeedbackIteration(
          iteration: iteration,
    ...
    return PresidentTransferAmbitionFeedbackReport(
      m20Baseline: m20Baseline,
