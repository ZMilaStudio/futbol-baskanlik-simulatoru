...
void main() {
  ...
  test('M16 sequential elections use post-handover incumbent reputation', () {
    ...
    expect(report.reelectionRate, inInclusiveRange(0.45, 0.85));
    expect(report.averageFinalMediaCredibility, inInclusiveRange(45, 85));
    expect(report.averageFinalIdentityTrust, inInclusiveRange(45, 80));
    expect(report.finalMediaStates.every((item) => item.credibility >= 0 && item.credibility <= 100), isTrue);
    expect(report.finalFanStates.every((item) => item.identityTrust >= 0 && item.identityTrust <= 100), isTrue);
  });
