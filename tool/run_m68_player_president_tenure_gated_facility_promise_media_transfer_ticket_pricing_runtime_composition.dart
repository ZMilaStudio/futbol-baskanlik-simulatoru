...
void main(List<String> args) {
  ...
  final m67ParityWithoutFacility =
  ...
  final fiveProviders = facility.calls == 1 &&
      promise.calls == 1 &&
      media.calls == 1 &&
      transfer.calls > 0 &&
      ticket.calls == 1;
  ...
  final baselineDecisions = {
    for (final item in aiBaseline.boundaries.single.source.decisions)
      item.clubId: item,
  };
  final playerDecisions = {
    for (final item in composed.boundaries.single.source.decisions)
      item.clubId: item,
  };
  final aiIds = baselineDecisions.keys.where((id) => id != controlled);
  final aiParity47 = aiIds.length == 47 &&
      aiIds.every(
        (id) => playerDecisions[id]!.signature == baselineDecisions[id]!.signature,
      );
  final facilityChanged = baselineDecisions[controlled]!.invested &&
      !playerDecisions[controlled]!.invested;
  ...
  final blockedFacility = _CountingHoldFacilityProvider();
  final blocked =
      PlayerPresidentTenureGatedFacilityPromiseMediaTransferTicketPricingRuntimeCareerEngine(
    facilityProvider: blockedFacility,
    ticketAiPolicy: const _AlwaysBalancedPolicy(),
  ).resume(
  ...
  final lostBlocksFacility =
      blockedFacility.calls == 0 && blocked.checkpoint.tenureControl.lost;
  ...
  if (!m67ParityWithoutFacility ||
      ...
      !saveResume) {
    throw StateError(
      'M68 canonical failure: m67Parity=$m67ParityWithoutFacility '
      'fiveProviders=$fiveProviders facilityChanged=$facilityChanged '
      'aiParity47=$aiParity47 lostBlocksFacility=$lostBlocksFacility '
      'saveResume=$saveResume',
    );
  }

  print(
    'M68_PLAYER_PRESIDENT_TENURE_GATED_FACILITY_PROMISE_MEDIA_TRANSFER_TICKET_PRICING_RUNTIME_COMPOSITION_PASS '
    'controlled=$controlled m67ParityWithoutFacility=$m67ParityWithoutFacility '
    'fiveProviders=$fiveProviders facilityChanged=$facilityChanged '
    'aiParity=47 lostBlocksFacility=$lostBlocksFacility '
